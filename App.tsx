...
import {
  ...
  HistoricNote, // Import HistoricNote
  ActiveView // Import ActiveView
} from './types';
...
import { 
  ...
  getStoredHistoricNotes, // Import history storage functions
...
import { SparklesIcon, LoadingSpinner, LightBulbIcon, MicrophoneIcon, CalculatorIcon } from './components/Icons';
...
const App: React.FC = () => {
  ...
  const [templates, setTemplates] = useState<Templates>(getStoredTemplates());
  ...
  useEffect(() => {
    // Load historic notes
    setHistoricNotes(getStoredHistoricNotes());

    const currentStoredTemplates = getStoredTemplates();
    let updated = false;
  ...
  const addNoteToHistory = (noteData: Omit<HistoricNote, 'id' | 'timestamp'>) => {
    ...
    const updatedHistory = addHistoricNoteEntry(newHistoricNote);
  ...
  const handleClearHistory = () => {
    if(window.confirm("¿Está seguro de que desea borrar todo el historial de notas? Esta acción no se puede deshacer.")) {
        addHistoricNoteEntry({} as HistoricNote); // This is a bit of a hack to use the logic to clear
        localStorage.removeItem('notasai_history'); // Make sure it's fully cleared
        setHistoricNotes([]);
    }
  };
  ...
  return (
    <div className="flex h-screen bg-neutral-100 dark:bg-dark-bg font-sans">
      ...
      <div className="flex-1 flex flex-col ml-64 overflow-hidden"> {/* Adjust ml to match sidebar width */}
        <header className="bg-white dark:bg-neutral-800 shadow-sm p-4 border-b dark:border-dark-border">
