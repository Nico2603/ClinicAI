...
import { createSimpleDeepgramService, SimpleDeepgramService } from '../lib/services/simpleDeepgramService';
...
export const useSimpleSpeech = (options: UseSimpleSpeechOptions = {}): UseSimpleSpeechReturn => {
  ...
  // Referencias
  const serviceRef = useRef<SimpleDeepgramService | null>(null);
  ...
  const createService = useCallback(() => {
    ...
    return createSimpleDeepgramService(apiKey, {
      onTranscript: (result) => {
        if (result.isFinal) {
          // Transcripción final
