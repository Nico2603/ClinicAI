...
const NoteDisplay: React.FC<NoteDisplayProps> = ({ note, onNoteChange, title = "Resultado Generado", isLoading = false, groundingMetadata }) => {
  ...
  if (isLoading) {
    return (
      <div className="mt-6 p-4 border border-neutral-200 dark:border-neutral-700 rounded-lg shadow-sm bg-white dark:bg-neutral-800 animate-pulse">
  ...
  if (!note && !isLoading) {
      return (
        <div className="mt-6 p-4 text-center text-neutral-500 dark:text-neutral-400 border border-dashed border-neutral-300 dark:border-neutral-600 rounded-lg bg-white dark:bg-neutral-800">
  ...
  return (
    <div className="mt-6 p-4 border border-neutral-200 dark:border-neutral-700 rounded-lg shadow-sm bg-white dark:bg-neutral-800">
      <div className="flex justify-between items-center mb-3">
        ...
        <div className="flex space-x-2">
          {onNoteChange && (
            <button
              onClick={toggleEdit}
              className="p-2 text-neutral-500 hover:text-primary dark:text-neutral-400 dark:hover:text-dark-primary rounded-full focus:outline-none focus:ring-2 focus:ring-primary dark:focus:ring-dark-primary transition-colors"
          ...
          <button
            onClick={handleCopy}
            className={`p-2 rounded-full focus:outline-none focus:ring-2 focus:ring-primary dark:focus:ring-dark-primary transition-colors ${
              copied 
                ? 'text-green-500 hover:text-green-600' 
                : 'text-neutral-500 hover:text-primary dark:text-neutral-400 dark:hover:text-dark-primary'
            }`}
      ...
      {isEditing && onNoteChange ? (
        ...
        <pre className="whitespace-pre-wrap text-sm text-neutral-700 dark:text-neutral-200 bg-neutral-50 dark:bg-neutral-700/40 p-4 rounded-md">{editableNote}</pre>
      )}
